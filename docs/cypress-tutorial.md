...
#### Using NPX (Recommended for Debugging)

NPX commands are useful for debugging while the browser is open:

**Debug in browser (interactive mode):**

```bash
# Open Cypress UI for debugging (no --spec flag in debug mode)
npx cypress open --project frontend/src/__tests__/cypress --browser chrome
```
...
npx cypress run --headed --browser chrome --project frontend/src/__tests__/cypress --spec '**/testManifestLinks.cy.ts'
```
...
```bash
# Run headless for CI
npx cypress run --browser chrome --project frontend/src/__tests__/cypress --spec '**/testManifestLinks.cy.ts'
```

#### Debugging with External Cluster and Frontend Changes

When debugging against an external cluster with local frontend changes, run the dev server with external cluster proxy enabled using `start:dev:ext`.

**Optional environment variables** (if not already set in Step 3):

```bash
export CY_RESULTS_DIR=/path/to/results/  # Custom location for screenshots/videos
export CY_WATCH=false                     # Disable auto-rerun on file changes
export CY_RETRY=0                         # No retries (see failures immediately)
```

**Option 1: One-liner (single terminal):**

```bash
# Kill any existing processes on ports 9001/4010, start dev server, then open Cypress
lsof -ti :9001,4010 | xargs -rt kill -9 ; (npm --prefix frontend run start:dev:ext &) && npm --prefix frontend run cypress:open
```

**Option 2: Two terminals (recommended for better control):**

**Terminal 1 - Start dev server:**

```bash
oc login -u <username> -p <password> -s <serverUrl>
lsof -ti :9001,4010 | xargs -rt kill -9
...
```bash
# Headless run
npm --verbose run --prefix frontend cypress:run -- -s '**/testName.cy.ts' --env skipTags='@Bug @Maintain',grepTags='@TagName'

# Or run in browser (headed)
npx cypress run --headed --browser chrome --project frontend/src/__tests__/cypress -s '**/testName.cy.ts' --env skipTags='@Bug @Maintain',grepTags='@TagName'

# Or open Cypress UI for debugging
npm --prefix frontend run cypress:open
```

For more details, see [docs/testing.md](./testing.md#cypress-tests).

---

## Quick Demo

A hands-on walkthrough: adding a test ID to a component, using it in a page object, and verifying it works in Cypress interactive mode.
